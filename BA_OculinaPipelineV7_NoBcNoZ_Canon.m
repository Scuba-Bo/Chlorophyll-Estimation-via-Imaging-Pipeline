...
%% Pipeline Step 2: Global Depth Value Assignment & Assume Backscatter Negligible
% GOAL: Assign depths to a chosen estimated value and set backscatter to 0.
% INPUT: 'chart_dataX.mat' (RGBs) 
...
% --- 2. OVERRIDE: CONSTANT DEPTH ---
all_depths = 0.4; % Your assumed 0.4m distance

% --- 3. INITIALIZE STORAGE ---
% Since you are skipping the regression loop, just force zero Bc
all_Bc = zeros(1, 3);

% --- 5. OVERRIDE: FORCE ZERO BACKSCATTER BEFORE SAVING ---
all_Bc = zeros(size(all_Bc)); % Reset all measured backscatter to zero

save('Backscatter_Depths.mat', 'all_depths', 'all_Bc');
disp('Step 2 Complete. (Backscatter set to 0 for this run).');

%% Pipeline Step 3: Backscatter Modeling & Subtraction
% Note: If backscatter is assumed to be negligible, this step doesn't
% change the image.
...
rgbFilename = 'IMG_2280.png'; % Ensure this matches your Step 1 file

fprintf('Loading Images...\n');
rgbImg = im2double(imread(rgbFilename));
depthImg = ones(size(rgbImg, 1), size(rgbImg, 2)) * 0.4;
...
% --- 3. OVERRIDE: SET BACKSCATTER TO ZERO ---
fprintf('Skipping modeling: Setting Backscatter Map to Zero...\n');


Bc_Map = zeros(size(rgbImg));
J_BS_Removed = rgbImg; % No subtraction performed
...
figure(2); clf;
subplot(1,3,1); imshow(rgbImg); title('1. Original (Non-Enhanced)');
subplot(1,3,2); imshow(Bc_Map);       title('2. Calculated Backscatter Map');
subplot(1,3,3); imshow(J_BS_Removed);  title('3. BS Removed (Non-Enhanced)');
...
%% Pipeline Step 4: Wideband Attenuation (Beta) - No-Backscatter Override
% GOAL: Calculate color attenuation (Beta) assuming backscatter is negligible.
% METHOD: Uses Eq 9 with W_c extracted from the 24% gray patch (Patch 17).
%         Beta = -ln(Raw_Underwater_Patch17 / Theoretical_patch17) / z
% INPUT: 'chart_data1.mat', 'Backscatter_Depths.mat' (Should have Bc=0)
...
% Theoretical reflectance of the DGK 24% patch (Neutral gray R=G=B=Y)
...
fprintf('--- Calculating Attenuation (Beta) - Backscatter Negligible Mode ---\n');

raw_water_24 = underwater_data.rgb_values(patch24Idx, :);
z  = all_depths(1); 

% Normalize intensity values (Handling 16-bit or 8-bit input)
...
% In this override, we assume Bc = 0, so the direct signal (Dc) is just the raw value.
Dc = water_val; 

% Calculate W_c (The ratio of light remaining at depth vs theoretical target)
Wc = Dc ./ theoretical_24;

% Calculate Beta (Attenuation Coefficient)
% Beta = -ln(Wc) / z
Beta = -log(Wc) / z;
...
sgtitle('Single-Chart Attenuation (\beta) - No Backscatter Correction', 'FontSize', 14);
...
for c = 1:3
    ...
    grid on; xlim([-0.2, z + 0.5]); ylim([0, 0.4]);
    
    text(z/2, theoretical_24(c)/2, sprintf('\\beta = %.3f', Beta(c)), ...
        'FontSize', 12, 'FontWeight', 'bold', 'Color', colors(c), 'HorizontalAlignment', 'center');
    
    fprintf('%s Channel: Target=%.3f, Water(Raw)=%.3f -> Beta = %.4f m^-1\n', ...
        channel_names{c}, theoretical_24(c), Dc(c), Beta(c));
end

save('step4_data.mat', 'Beta');
disp('Step 4 Complete. Beta values saved (Calculated without backscatter subtraction).');
...
% --- 1. SETTINGS ---
%depthFilename = 'IMG_2253.tif'; Depth file not used here
...
depthImg = ones(size(J_BS_Removed, 1), size(J_BS_Removed, 2)) * 0.4;
...
figure(2); clf;
subplot(1,2,1); imshow(J_BS_Removed); title('Original Image');
subplot(1,2,2); imshow(J_Display); title('Restored Colors');

save('Final_Scientific_Image.mat', 'J_Final', 'Scaling_Factors');
imwrite(J_Display, 'Restored_Reflectance_Theoretical.png');

%% Pipeline Step 6: Universal XYZ Transformation (Water-Corrected -> Theoretical)
...
%       using the actual spectral reflectances, camera spectral signature, and illuminant.
...
% 1.1 Load Data
% Canon-PowerShot-G7-X-Mark-III Signature (No headers in the CSV)
...
title('Theoretical Value Alignment (Black=True, Red=Water Corrected)');
view(45, 45);
